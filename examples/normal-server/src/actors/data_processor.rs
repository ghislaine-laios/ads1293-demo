...
impl DataProcessor {
    pub fn new() -> (Self, impl Stream<Item = Result<Bytes, DataProcessingError>>) {
...
impl DataProcessor {
    ...
    async fn notify_err(
        &mut self,
        err: DataProcessingError,
    ) -> Result<(), ErrorNotificationFailed> {
        self.ws_sender
            .send(Err(err))
            .await
...
pub(super) mod actions {
    use super::{Data, DataProcessingError};
    use actix_http::ws::{self};
    ...
    #[derive(Debug)]
    pub struct SaveToDatabase(pub Data);

    #[derive(Debug)]
    pub struct SendToPeer(pub Result<ws::Message, DataProcessingError>);
...
pub mod handlers {
    use super::{actions, ConnectionStatus, DataProcessor, SendToPeerError};
        ...
        async fn handle(&mut self, action: actions::AddRaw) -> Self::Output {
            ...
            let result = match frame {
                ...
                actix_http::ws::Frame::Binary(_) => {
                    return Err(DataProcessingError::NotSupportedFrame("binary".to_string()))
                }
                actix_http::ws::Frame::Continuation(_) => {
                    return Err(DataProcessingError::NotSupportedFrame(
